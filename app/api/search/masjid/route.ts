...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!query.trim()) {
      return NextResponse.json({ results: [], total: 0, totalPages: 0, currentPage: 1, hasMore: false, query: '' })
    }
    ...
    return NextResponse.json({ error: 'Failed to search masjids' }, { status: 500 })
  }
}